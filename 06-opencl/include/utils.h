...
class ClErrorChecker {
  public:
    void operator=(cl_int result) {
        if(!checked) {
            throw std::runtime_error("Last error value was not checked");
        }
        last_value = result;
        checked = false;
    }
    ...
    void operator()() {
        if (last_value != CL_SUCCESS) {
            throw std::runtime_error(getErrorString(last_value));
        }
        checked = true;
    }
...
std::string load_file(const std::string& filepath) {
    static const char* kernel_dir = STRINGIFY(OPENCL_KERNEL_DIR);

    std::ifstream file(kernel_dir + std::string("/") + filepath);
...
int opencl_print_build_log(cl_program program, cl_device_id device_id) {
    ...
    if (status != CL_SUCCESS) {
        printf("clGetProgramBuildInfo (%d)", status);
        goto fail_exit;
    }
